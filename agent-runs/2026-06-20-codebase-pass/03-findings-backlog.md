...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| F-001 | P2 | Bug | Open | Editor block insertion | Slash-palette insertion can create duplicate `order` values and place the new block after the wrong sibling. | `src/app/(app)/board/[id]/page.tsx:175-188` sets new block `order` to `currentIndex + 1`; `src/stores/board-store.ts:154-164` appends the block without shifting/renumbering existing blocks. | Medium: keyboard/slash insertion after a selected block can render in an unexpected position until the user manually reorders. | Small | Add store unit test for mid-list insertion; run targeted Vitest, lint, typecheck/build as needed. | Fix first. |
| F-002 | P2 | Bug | Open | Public analytics | Public pages ignore `board.analytics.enabled` before tracking views/clicks. | `src/components/public-board/public-board-client.tsx:18-23` always calls `trackView`/`trackClick`; `src/hooks/use-analytics.ts:20-71` always writes analytics/click docs. | Medium: owners cannot disable visitor tracking despite board-level setting. | Small | Guard public tracking with `board.analytics.enabled`; lint/typecheck/build. | Queue after F-001 or defer if scope needs one fix. |
